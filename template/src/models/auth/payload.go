...
type Payload struct {
	...
	AdminType           string `json:"at,omitempty"`
...
// GenClientAdminJwt will generate client admin jwt token by a given admin and clientID
func GenClientAdminJwt(admin orm.Admin, clientID uint64) string {
	payload := Payload{
		AdminID:   admin.ID,
...
// GenSystemAdminJwt will generate system admin jwt token by a given admin
func GenSystemAdminJwt(admin orm.Admin) string {
	payload := Payload{
		AdminID:   admin.ID,
		AdminType: admin.AdminType,
	}

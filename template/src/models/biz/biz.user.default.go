...
func AuthorizeWXPhoneNumber(rawData vo.WXData, userID uint64) (*orm.User, error) {
	...
	wechatUser, err := ormWechatUser.ExistByOpenID(user.WechatOpenID)
	if err != nil {
		return nil, err
	}

	if jsonStr, err := enc.DecryptWXData(wechatUser.SessionKey, rawData.IV, rawData.EncryptedData); err != nil {
		return nil, err
	} else {
		fmt.Println(jsonStr)
		// return jsonStr, nil
...
func SaveWXUserInfo(rawData vo.WXData, userID uint64, openID string) (*orm.User, error) {
	...
	if err := user.Get(&user, userID); err != nil {
		...
		if err := user.FindByWechatOpenID(&user, openID); err != nil {
			// could not find by openid, then create user
			// TODO: handle unionID
			user = orm.User{
				WechatOpenID: openID,
			}
	...
	// get wechatUser by openID
	var ormWechatUser orm.WechatUser
	wechatUser, err := ormWechatUser.ExistByOpenID(openID)
	if err != nil {
		return nil, err
	}

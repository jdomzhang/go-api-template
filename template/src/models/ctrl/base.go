...
// ShouldBeAdmin will check current login context, and make sure it's an authorized admin
func ShouldBeAdmin(c *gin.Context) {
	loginContext := getLoginContext(c)
	if ok := loginContext.IsValidAdmin; !ok {
		c.AbortWithError(http.StatusUnauthorized, errors.New("current user has no proper right to access this api"))
	}
}
...
func getLoginContext(c *gin.Context) LoginContext {
	...
	loginContext := LoginContext{
		...
		IsValidAdmin: payload.UserID > 0 && ok && strings.Contains(payload.Roles, "|admin|"),
	}
